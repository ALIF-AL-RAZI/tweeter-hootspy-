...
export const fetchScraps = createAsyncThunk('fetchScraps/fetchScraps', ({ id, token, qry = '' }) =>
    axios.get(`${TweetsyConfig.getNodeUrl()}email-scrapper/scrap-by-extension/${id}?${qry}`, {
...
export const startScrap = createAsyncThunk('scrapedResults/startScrap', ({ id, token }) => {
    ...
    return axios.get(`${TweetsyConfig.getNodeUrl()}email-scrapper/extensions-scrap-start/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
    });
});
